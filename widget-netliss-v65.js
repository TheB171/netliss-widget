...
function nlEstadoRestaurar(){
  try{
    ...
    NL.revId = e.revId; NL.afiId = e.afiId;
...
function nlVoltarAoMenu(){
  if(NL.revId){ nlMenuRevendedor(); return; }
...
var NL_UTIL = ["nlMsgLiss","nlMsgUser","nlChips","nlCard","nlForm","nlRolar","nlEscapar","nlSalvar","nlLer",
  ...
  "nlCopiarFallback","nlCopiarLegenda","nlCopiarOrientacao","nlIniciarPolling",
  "nlCarregarAvisos","nlNomeFn","nlTemAcao","nlRefazerBotoes","nlRefazerTela","nlResgate",
  "nlFimSuporte","nlFimPergunta","nlPlanoId",
  "nlFichaLer","nlFichaGravar","nlTocar","nlOcioso","nlExpirarPorInatividade","nlNivelOk","nlValorNum","nlMoeda","nlCopiarTudoRev","nlAvisinho","nlCartaoAfiliado","nlCronoHtml","nlPintarCrono","nlPararCrono","nlCronoAcabou","nlIniciarCrono","nlRetomarPagamento"];
    ...
    nlMsgLiss(trata + "ercebi que você já usou os <b>3 testes</b> que cada pessoa tem direito.", function(){
      nlMsgLiss("Para continuar com a internet ilimitada, agora o caminho é escolher um plano.", function(){
        nlChips([
          ...
          { rotulo:"Ganhe dinheiro com a Netliss", largo:true, acao:nlGanharDinheiro }
        ]);
      });
    });
    return;
...
function nlSalvarNome(){
  ...
  if(n.length < 2){ NL.travas = NL.travas || {}; for(var _tk in NL.travas){ NL.travas[_tk] = 0; } alert("Digite seu nome"); return; }
  NL.nome = n.split(" ")[0];
...
function nlRevTim(){ nlRevModalidade("TIM"); }
function nlRevVivo(){ nlRevModalidade("Vivo"); }
function nlRevClaro(){ nlRevMostrarRegra("CLARO"); }
function nlRevPre(){ nlRevMostrarRegra("PRE"); }
function nlRevPos(){ nlRevMostrarRegra("POS"); }
...
function nlRevLinkApp(){
  ...
  nlMsgLiss("Aqui está o link oficial do nosso aplicativo:<br><br>" + CFG.playStore + "<br><br>Você pode copiar o link para enviar ao seu cliente, ou baixar o aplicativo aqui mesmo, se for você que precisa instalar.", function(){
    nlChips([
      { rotulo:"Copiar link do aplicativo", manter:true, acao:function(){ nlCopiarTexto(CFG.playStore); nlAvisinho("link copiado"); } },
      { rotulo:"Baixar o aplicativo", manter:true, acao:function(){ window.open(CFG.playStore, "_blank"); } },
...
function nlEnviarRecuperacao(){
  ...
  if(email.indexOf("@") < 1 || email.indexOf(".") < 3){ NL.travas = NL.travas || {}; for(var _tk in NL.travas){ NL.travas[_tk] = 0; } alert("Digite um e-mail válido"); return; }
  if(!bruta || bruta.length < 10){ NL.travas = NL.travas || {}; for(var _tk in NL.travas){ NL.travas[_tk] = 0; } alert("Escolha a data da compra"); return; }
...
function nlValidarDadosPix(){
  ...
  if(nome.length < 5){ NL.travas.cobranca = 0; alert("Digite seu nome completo"); return; }
  if(email.indexOf("@") < 1 || email.indexOf(".") < 3){ NL.travas.cobranca = 0; alert("Digite um e-mail válido"); return; }
  if(cpf.length !== 11){ NL.travas.cobranca = 0; alert("O CPF precisa ter 11 números"); return; }
...
function nlPagarPix(){
  nlDigitar(async function(){
    ...
    var corpo = { acao: acaoPg, metodo:"pix", plan_id: nlPlanoId(p), nome: NL.pgNome, email: NL.pgEmail, cpf: NL.pgCpf, ref: (NL.modoRenov ? "" : (nlLer("netliss_ref") || "")) };
...
function nlChamadoPagamento(){
  ...
  nlGarantirTicket();
  nlApiFogo({ acao:"chamado_abrir", nome: NL.pgNome || "", email: NL.pgEmail || "",
    whatsapp: NL.pgZap || "",
...
async function nlVerificarPagamento(quieto){
  ...
  await (async function(){
    ...
    if(s.status === "approved" || s.status === "paid" || s.status === "confirmed"){
      ...
      nlApiFogo({ acao:"venda", plano: plano.nome, valor: plano.preco, valor_num: nlValorNum(plano.preco), usuario: NL.vendaUser, metodo: NL.metodoPg || "pix", ref: (NL.modoRenov ? "" : (nlLer("netliss_ref") || "")), email: NL.pgEmail || "", cpf: NL.pgCpf || "" });
      var cred = "";
      var _ehRev = NL.revId ? true : false;
      if(_ehRev && s.username && s.password){
        NL.copiaUser = s.username; NL.copiaSenha = s.password;
        NL.copiaDados = "Usuario: " + s.username + String.fromCharCode(10) + "Senha: " + s.password + String.fromCharCode(10) + "Validade: " + (s.expires_at || s.validade || "");
        cred = "Usuário: <b>" + nlEscapar(s.username) + "</b><br>Senha: <b>" + nlEscapar(s.password) + "</b><br>Validade: <b>" + nlEscapar(s.expires_at || s.validade || "") + "</b><br><br>Esses dados também foram enviados por e-mail para o seu cliente.<br><br><button class='chip' data-vivo='1' onclick='nlCopiarCred(0,this)'>Copiar usuário</button> <button class='chip' data-vivo='1' onclick='nlCopiarCred(1,this)'>Copiar senha</button> <button class='chip' data-vivo='1' onclick='nlCopiarTudoRev(this)'>Copiar tudo</button>";
      } else {
        cred = "O acesso foi enviado para o e-mail <b>" + nlEscapar(NL.pgEmail) + "</b>.<br><br>Confira também a caixa de spam ou promoções.";
      }
...
function nlValidarDadosCartao(){
  ...
  var email = _ce.value.trim();
  var cpf = nlSoNumeros(_cc.value);
  if(nome.length < 5){ NL.travas = NL.travas || {}; for(var _tk in NL.travas){ NL.travas[_tk] = 0; } alert("Digite seu nome completo"); return; }
  if(email.indexOf("@") < 1 || email.indexOf(".") < 3){ NL.travas = NL.travas || {}; for(var _tk in NL.travas){ NL.travas[_tk] = 0; } alert("Digite um e-mail válido"); return; }
  if(cpf.length !== 11){ NL.travas = NL.travas || {}; for(var _tk in NL.travas){ NL.travas[_tk] = 0; } alert("O CPF precisa ter 11 números"); return; }
  NL.pgNome = nome; NL.pgEmail = email; NL.pgCpf = cpf;
  nlMsgUser(nome);
  nlApiFogo({ acao:"nome", nome:nome, email:email });
  nlFormDadosCartao();
...
function nlTokenizarEPagar(){
  ...
  if(num.length < 13){ NL.travas = NL.travas || {}; for(var _tk in NL.travas){ NL.travas[_tk] = 0; } alert("Confira o número do cartão"); return; }
  if(titular.length < 5){ NL.travas = NL.travas || {}; for(var _tk in NL.travas){ NL.travas[_tk] = 0; } alert("Digite o nome impresso no cartão"); return; }
  var partes = val.split("/");
  if(partes.length !== 2 && val.length === 4){ partes = [val.substring(0,2), val.substring(2,4)]; }
  if(partes.length !== 2 || partes[0].length !== 2){ NL.travas = NL.travas || {}; for(var _tk in NL.travas){ NL.travas[_tk] = 0; } alert("Validade no formato MM/AA"); return; }
  if(cvv.length < 3){ NL.travas = NL.travas || {}; for(var _tk in NL.travas){ NL.travas[_tk] = 0; } alert("Confira o CVV"); return; }
  var mes = partes[0];
...
function nlPagarCartao(tokenCartao){
  nlDigitar(async function(){
    var p = CFG.planos[NL.planoAtual];
    var acaoPg = (NL.modoRenov && NL.contaRenov) ? "pagamento_renovar" : "pagamento_criar";
    var corpo = { acao: acaoPg, metodo:"credit_card", card_token: tokenCartao, plan_id: nlPlanoId(p), nome: NL.pgNome, email: NL.pgEmail, cpf: NL.pgCpf, ref: (NL.modoRenov ? "" : (nlLer("netliss_ref") || "")),
...
function nlRecompensaRegras(){
  ...
  nlMsgLiss("Mostra a Netliss funcionando no seu celular e ganhe:<br><br>&#128241; <b>Print do app conectado</b> (postado no feed ou reels) = <b>+10 dias</b> no seu plano.<br><br>&#127909; <b>Vídeo ou gravação da tela</b> usando e comentando (feed ou reels) = <b>+1 acesso completo</b>.<br><br>Importante: precisa ser postado no <b>feed ou reels</b> (não vale só stories) e com a legenda que eu já deixo pronta pra você.", function(){
    nlMsgLiss("Aqui está a legenda — é só copiar e colar na sua postagem:<br><br><i>" + nlEscapar(CFG.legendaPost) + "</i><br><br><button class='chip' data-vivo='1' onclick='nlCopiarLegenda(this)'>Copiar legenda</button>", function(){
...
function nlEnviarInsta(){
  if(nlTrancado("insta")){ return; }
  var insta = document.getElementById("nl-insta").value.trim();
  insta = insta.split(" ").join("");
  if(insta.length < 2){ NL.travas = NL.travas || {}; for(var _tk in NL.travas){ NL.travas[_tk] = 0; } alert("Digite o seu @ do Instagram"); return; }
...
function nlVerificarUsuario(){
  ...
  if(u.length < 3){ NL.travas = NL.travas || {}; for(var _tk in NL.travas){ NL.travas[_tk] = 0; } alert("Digite seu nome de usuário"); return; }
  nlMsgUser(u);
  NL.usuarioApp = u;
  nlDigitar(async function(){
...
function nlCopiarTudoRev(b){
  nlCopiarTexto(NL.copiaDados || "");
  if(b){ b.textContent = "Copiado!"; setTimeout(function(){ try{ b.textContent = "Copiar tudo"; }catch(e){} }, 2000); }
}
function nlMoeda(n){
  var v = Number(n || 0);
  var inteiro = Math.floor(v);
  var cent = Math.round((v - inteiro) * 100);
  if(cent === 100){ inteiro = inteiro + 1; cent = 0; }
  var si = String(inteiro);
  var saida = "";
  var c = 0;
  for(var i = si.length - 1; i >= 0; i--){
    saida = si.charAt(i) + saida;
    c++;
    if(c % 3 === 0 && i > 0){ saida = "." + saida; }
  }
  return "R$ " + saida + "," + (cent < 10 ? "0" : "") + cent;
}
...
function nlNivelOk(n){
  var v = String(n || "BASICA").toUpperCase();
  if(v.indexOf("VIP") > -1){ return "VIP"; }
  if(v.indexOf("TOP") > -1){ return "TOP"; }
  return "BASICA";
}
function nlComprarLicenca(nivel){
  NL.nivelLicenca = nlNivelOk(nivel);
  NL.planoAtual = "LIC_" + NL.nivelLicenca;
...
function nlVerificarRevendedor(){
  if(nlTrancado("rev")){ return; }
  var id = document.getElementById("nl-rev-id").value.trim();
  if(id.length < 2){ NL.travas = NL.travas || {}; for(var _tk in NL.travas){ NL.travas[_tk] = 0; } alert("Digite o seu ID de revendedor"); return; }
  NL.revId = id;
  nlMsgUser(id);
  nlDigitar(async function(){
    var r = await nlApi({ acao:"revendedor_verificar", revid: id });
    if(!r){ nlInstabilidade(nlFluxoRevendedor); return; }
...
function nlMenuRevendedor(dias){
  ...
  nlMsgLiss(cab, function(){
    nlChips([
      ...
      { rotulo:"Copiar link do app", manter:true, acao:function(){ nlCopiarTexto(CFG.playStore); nlAvisinho("link copiado, é só enviar ao seu cliente"); } },
...
function nlRevRegras(){
  nlMsgLiss("Qual é a operadora do seu cliente?", function(){
    nlChips([
      { rotulo:"TIM", acao:nlRevTim },
      { rotulo:"Vivo", acao:nlRevVivo },
      { rotulo:"Claro", acao:nlRevClaro },
...
function nlRevOutras(){
  nlMsgLiss("A nossa base são as redes da <b>TIM</b>, da <b>Vivo</b> e da <b>Claro</b>. Se a operadora do seu cliente usa a rede de uma delas, tem tudo para funcionar normalmente.", function(){
    nlMsgLiss("Caso não saiba, pesquise no Google sobre qual rede a operadora dele usa. Normalmente muitas operadoras possuem nome diferente, mas utilizam a mesma rede.", function(){
      nlChips([
        { rotulo:"Usa a rede da TIM", acao:nlRevTim },
        { rotulo:"Usa a rede da Vivo", acao:nlRevVivo },
        { rotulo:"Usa a rede da Claro", acao:nlRevClaro },
        { rotulo:"Voltar ao menu", acao:nlMenuRevendedor }
      ]);
    });
  });
...
function nlRevModalidade(op){
  nlMsgLiss("Anotado: <b>" + op + "</b>.", function(){
    nlMsgLiss("E qual é a modalidade do chip dele?", function(){
      nlChips([
        { rotulo:"Pré-pago", acao:nlRevPre },
        { rotulo:"Pós-pago ou Controle", acao:nlRevPos },
        { rotulo:"Voltar ao menu", acao:nlMenuRevendedor }
      ]);
    });
  });
...
function nlRevMostrarRegra(chave){
  ...
  nlMsgLiss("Manda essa orientação para o seu cliente:", function(){
    nlMsgLiss(t, function(){
      nlChips([
        { rotulo:"Copiar orientação", manter:true, acao:function(){ nlCopiarTexto(NL.regraCopiar); nlAvisinho("orientação copiada, é só enviar ao seu cliente"); } },
  ...
  nlDigitar(async function(){
    ...
    if(r && r.usuario){
      ...
      nlMsgLiss("Prontinho! Teste grátis gerado para o seu cliente:<br><br>Usuário: <b>" + nlEscapar(r.usuario) + "</b><br>Senha: <b>" + nlEscapar(r.senha) + "</b><br>Validade: <b>" + nlEscapar(r.validade_horas || "1") + " hora</b><br><br>Clique em Copiar dados que eu monto a mensagem inteira, com usuário, senha, validade e as instruções de como conectar — pronto para enviar ao seu cliente.", function(){
        nlChips([
          { rotulo:"Copiar dados", manter:true, acao:function(){ nlCopiarTexto(NL.copiaDados); nlAvisinho("dados copiados, é só enviar ao seu cliente"); } },
...
function nlRevComprarAcesso(){
  ...
  NL.planoAtual = "ACESSO_" + nlNivelOk(NL.nivelLicenca);
...
function nlRevSeguirCompra(){
  if(nlTrancado("revcompra")){ return; }
  var e = document.getElementById("nl-rev-email-cli").value.trim();
  if(e.indexOf("@") < 1 || e.indexOf(".") < 3){ NL.travas = NL.travas || {}; for(var _tk in NL.travas){ NL.travas[_tk] = 0; } alert("Digite um e-mail válido"); return; }
  nlMsgUser(e);
  NL.emailCliente = e;
  nlMostrarMetodos();
}
...
function nlVerificarAfiliado(){
  if(nlTrancado("afi")){ return; }
  var id = document.getElementById("nl-afi-id").value.trim();
  if(id.length < 2){ NL.travas = NL.travas || {}; for(var _tk in NL.travas){ NL.travas[_tk] = 0; } alert("Digite o seu ID de afiliado"); return; }
  nlMsgUser(id);
  nlDigitar(async function(){
    var r = await nlApi({ acao:"afiliado_consultar", afiid: id });
    if(!r){ nlInstabilidade(nlFluxoAfiliado); return; }
    if(r && r.success){
      nlSalvar("netliss_afiid", id, CFG.ttlId);
      NL.afiId = id;
      var link = "https://www.netliss.com/?ref=" + id;
      var _vend = Number(r.vendido || 0);
      var _pago = Number(r.pago || 0);
      var _com = _vend * 0.2;
      var _rec = _com - _pago;
      if(_rec < 0){ _rec = 0; }
      nlMsgLiss("Oi, <b>" + nlEscapar(r.nome || "") + "</b>! Aqui está o seu resumo:<br><br>" +
        "Visitantes pelo seu link: <b>" + (r.visitantes || 0) + "</b><br>" +
        "Testes grátis liberados: <b>" + (r.testes || 0) + "</b><br>" +
        "Clientes que contrataram: <b>" + (r.vendas || 0) + "</b><br>" +
        "Total vendido: <b>" + nlMoeda(_vend) + "</b><br><br>" +
        "Sua comissão (20%): <b>" + nlMoeda(_com) + "</b><br>" +
        "Já recebido: <b>" + nlMoeda(_pago) + "</b><br>" +
        "A receber: <b>" + nlMoeda(_rec) + "</b>", function(){
        nlCartaoAfiliado(link);
        nlChips([
          { rotulo:"Copiar meu link", manter:true, acao:function(){ nlCopiarTexto(link); nlAvisinho("link copiado"); } },
          { rotulo:"Baixar meu QR Code", manter:true, acao:function(){ nlBaixarQr(link); } },
...
function nlBuscarIdAfiliado(){
  if(nlTrancado("afibusca")){ return; }
  var b = document.getElementById("nl-afi-busca").value.trim();
  if(b.length < 4){ NL.travas = NL.travas || {}; for(var _tk in NL.travas){ NL.travas[_tk] = 0; } alert("Digite o WhatsApp ou a chave PIX"); return; }
  nlMsgUser(b);
  nlDigitar(async function(){
    var r = await nlApi({ acao:"afiliado_consultar", busca: b });
    if(!r){ nlInstabilidade(nlPerdiIdAfiliado); return; }
...
function nlBaixarQr(link){
  var url = "https://api.qrserver.com/v1/create-qr-code/?size=500x500&data=" + encodeURIComponent(link);
  var abrir = function(){ window.open(url, "_blank"); };
  try{
    if(!window.fetch || !window.URL || !URL.createObjectURL){ abrir(); return; }
    fetch(url).then(function(r){
      ...
      nlAvisinho("QR Code baixado");
    })["catch"](function(){ abrir(); });
  }catch(e){ abrir(); }
}
function nlCartaoAfiliado(link){
  var img = "https://api.qrserver.com/v1/create-qr-code/?size=460x460&margin=12&data=" + encodeURIComponent(link);
  nlCard("<div class='titulo' style='text-align:center'>Seu material de divulgação</div>" +
    "<img src='" + img + "' alt='QR Code' style='width:190px;display:block;margin:12px auto;border-radius:12px;background:#fff;padding:8px'>" +
    "<div class='pix-codigo'>" + nlEscapar(link) + "</div>");
}
function nlCriarAfiliado(){
  if(nlTrancado("aficriar")){ return; }
  ...
  if(pix.length < 4){ NL.travas = NL.travas || {}; for(var _tk in NL.travas){ NL.travas[_tk] = 0; } alert("Digite a sua chave PIX"); return; }
  if(nome.length < 5){ NL.travas = NL.travas || {}; for(var _tk in NL.travas){ NL.travas[_tk] = 0; } alert("Digite o nome completo do titular"); return; }
  if(banco.length < 2){ NL.travas = NL.travas || {}; for(var _tk in NL.travas){ NL.travas[_tk] = 0; } alert("Digite o banco"); return; }
  if(!tel){ NL.travas = NL.travas || {}; for(var _tk in NL.travas){ NL.travas[_tk] = 0; } alert("Preencha o DDD e o número do WhatsApp"); return; }
  nlMsgUser(nome);
  nlDigitar(async function(){
    var r = await nlApi({ acao:"afiliado_criar", pix:pix, nome:nome, banco:banco, whatsapp:tel });
    if(!r){ nlInstabilidade(nlAfiliadoCadastro); return; }
    if(r && r.success && r.afiid){
      nlSalvar("netliss_afiid", r.afiid, CFG.ttlId);
      var link = "https://www.netliss.com/?ref=" + r.afiid;
      nlMsgLiss("Pronto! Esse é o seu material:<br><br>O SEU <b>ID DE AFILIADO</b> é: <b>" + nlEscapar(r.afiid) + "</b> — guarde bem, é com ele que você entra aqui.<br><br>Como funciona o seu ganho: você recebe <b>20% da PRIMEIRA compra</b> de cada cliente que vier pelo seu link. Não há mensalidade e não há comissão recorrente nas renovações dele. E o pagamento é <b>diário</b>: tudo o que você vender hoje cai na sua chave PIX amanhã.", function(){
        nlCartaoAfiliado(link);
        nlChips([
          { rotulo:"Copiar meu link", manter:true, acao:function(){ nlCopiarTexto(link); nlAvisinho("link copiado"); } },
          { rotulo:"Baixar meu QR Code", manter:true, acao:function(){ nlBaixarQr(link); } }
...
function nlEnviarChamado(){
  ...
  if(em.indexOf("@") < 1 || em.indexOf(".") < 3){ NL.travas = NL.travas || {}; for(var _tk in NL.travas){ NL.travas[_tk] = 0; } alert("Digite um e-mail válido"); return; }
  if(zap.length < 10){ NL.travas = NL.travas || {}; for(var _tk in NL.travas){ NL.travas[_tk] = 0; } alert("Digite o WhatsApp com DDD"); return; }
  nlMsgUser(em + " · " + zap);
  nlDigitar(function(){
...
function nlBootWidget(){
  ...
  if(_r.ref){ try{ nlSalvar("netliss_ref", _r.ref, CFG.ttlId); }catch(e){} }
