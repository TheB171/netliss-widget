...
var CFG = {
  ...
  difyUrl: "https://atendimento.netliss.com/v1",
  difyKey: "app-SFg20AZXgDtVBoESrNZWZScT",
...
async function nlEnviarTexto(){
  ...
  nlRolar();
  try{
    var payload = { inputs: {}, query: txt, response_mode: "blocking", user: NL.clientId };
    if(NL.difyConv){ payload.conversation_id = NL.difyConv; }
    var resp = await fetch(CFG.difyUrl + "/chat-messages", {
      method: "POST",
      headers: { "Authorization": "Bearer " + CFG.difyKey, "Content-Type": "application/json" },
      body: JSON.stringify(payload)
    });
    var d = await resp.json();
    t.remove();
    if(d && d.answer){
      if(d.conversation_id && !NL.difyConv){ NL.difyConv = d.conversation_id; nlSalvarConv(); }
      var d2 = document.createElement("div");
      d2.className = "msg liss";
      d2.innerHTML = nlFormatarLiss(d.answer);
