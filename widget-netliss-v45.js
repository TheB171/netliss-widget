...
function nlContinuar(origem){
  ...
  nlMsgLiss("Oi de novo, " + trata + "continuamos de onde paramos!", function(){
    var b = [];
    if(NL.pgId){ b.push({ rotulo:"Já paguei, verificar", acao:nlVerificarPagamento }); }
    if(NL.copiaUser || NL.usuarioApp || NL.vendaUser){
      b.push({ rotulo:"Quero contratar", acao:nlMostrarPlanos });
      b.push({ rotulo:"Preciso de ajuda", acao:nlSuporteNovo });
    } else {
      b.push({ rotulo:"Continuar meu atendimento", acao:nlFluxoNovo });
    }
    b.push({ rotulo:"Recomeçar do início", sec:true, acao:function(){ nlReiniciar(); } });
    nlChips(b);
  });
}
  ...
  nlDigitar(async function(){
    ...
    if(r && r.success && r.pix){
      nlMsgLiss("Aqui está o seu PIX do <b>" + p.nome + "</b> (" + p.preco + "). Escaneie o QR Code ou copie o código PIX.<br><br>Assim que pagar, é só clicar no botão abaixo que eu confirmo na hora.", function(){
        ...
        nlChips([
          { rotulo:"Já paguei, verificar", largo:true, acao:nlVerificarPagamento },
          { rotulo:"Copiar código PIX", acao:function(){ nlCopiarPix(); } }
        ]);
