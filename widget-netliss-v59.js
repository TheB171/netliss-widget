...
function nlChips(opcoes){
  ...
  try{
    ...
    for(var _q = 0; _q < opcoes.length; _q++){
      ...
      var _fn = nlNomeFn(_f);
      NL.ultimosChips.push({ r: opcoes[_q].rotulo, f: _fn, s: opcoes[_q].sec ? 1 : 0, g: opcoes[_q].largo ? 1 : 0 });
    }
    nlEstadoSalvar();
  }catch(e){}
...
function nlRefazerBotoes(e){
  ...
  for(var i = 0; i < e.botoes.length; i++){
    ...
    if(fn){ b.push({ rotulo: e.botoes[i].r, sec: e.botoes[i].s ? true : false, largo: e.botoes[i].g ? true : false, acao: fn }); }
  }
...
var NL_UTIL = ["nlMsgLiss","nlMsgUser","nlChips","nlCard","nlForm","nlRolar","nlEscapar","nlSalvar","nlLer",
  ...
  "nlCopiarFallback","nlCopiarLegenda","nlCopiarOrientacao","nlDesativarJaPaguei","nlIniciarPolling",
  "nlCarregarAvisos","nlNomeFn","nlTemAcao","nlRefazerBotoes","nlRefazerTela","nlResgate",
  "nlFimSuporte","nlFimPergunta","nlPlanoId",
  "nlFichaLer","nlFichaGravar","nlTocar","nlOcioso","nlExpirarPorInatividade","nlNivelOk","nlValorNum"];
