...
function nlChips(opcoes){
  ...
  try{
    ...
    for(var _q = 0; _q < opcoes.length; _q++){
      ...
      NL.ultimosChips.push({ r: opcoes[_q].rotulo, f: (_f && _f.name) ? _f.name : "", s: opcoes[_q].sec ? 1 : 0, g: opcoes[_q].largo ? 1 : 0 });
    }
    nlEstadoSalvar();
  }catch(e){}
  ...
  for(var i=0;i<opcoes.length;i++){
    (function(op){
      ...
      if(op.acao && op.acao.name && typeof window[op.acao.name] === "function"){
        b.setAttribute("onclick", "nlClique(this, '" + op.acao.name + "')");
      } else if(op.acao){
        var fonte = String(op.acao);
        var cmd = "nlMsgUser(this.getAttribute('data-r'));(" + fonte + ")()";
        b.setAttribute("onclick", cmd.split('"').join("&quot;"));
      }
      box.appendChild(b);
    })(opcoes[i]);
  }
...
function nlAbrirChat(origem){
  ...
  if(!NL.jaAbriu){
    ...
    if(_h.length && _t && !NL.encerrado){ nlContinuar(origem); }
...
function nlReiniciar(){
  ...
  try{ nlSalvar("netliss_estado", { tela: "", botoes: [], telaAtual: "", telaArgs: [] }, 2592000000); }catch(e){}
  nlHistRestaurar();
  nlInicio();
}
...
function nlContinuar(origem){
  nlEstadoRestaurar();
  NL.ticket = nlLer("netliss_ticket");
  var e = nlLer("netliss_estado") || {};
  if(e.tela){
    NL.corpo.innerHTML = e.tela;
    nlRolar();
    setTimeout(nlRolar, 150);
    var temBotao = NL.corpo.querySelector(".chips button");
    if(temBotao){ return; }
    var refeito = false;
    if(!refeito && e.telaAtual && typeof window[e.telaAtual] === "function"){
      try{
        NL.silencioso = true;
        window[e.telaAtual].apply(null, e.telaArgs || []);
        NL.silencioso = false;
    ...
    if(!refeito && e.botoes && e.botoes.length){
      var b = [];
