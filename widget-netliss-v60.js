...
var NL_UTIL = ["nlMsgLiss","nlMsgUser","nlChips","nlCard","nlForm","nlRolar","nlEscapar","nlSalvar","nlLer",
  ...
  "nlFichaLer","nlFichaGravar","nlTocar","nlOcioso","nlExpirarPorInatividade","nlNivelOk","nlValorNum","nlAvisinho","nlCartaoAfiliado","nlCronoHtml","nlPintarCrono","nlPararCrono","nlCronoAcabou","nlIniciarCrono","nlRetomarPagamento"];
...
async function nlAcessoSocorro(){
  ...
  nlDigitar(async function(){
    var r = await nlApi({ acao:"pagamento_status", payment_id: String(pend.id), email: NL.pgEmail || "" });
    var conta = (r && r.account) ? r.account : {};
    if(conta.username){
      nlPararCrono();
      nlLimparPendente();
...
function nlVerificarAfiliado(){
  ...
  nlDigitar(async function(){
    ...
    if(r && r.success){
      ...
      nlMsgLiss("Oi, <b>" + nlEscapar(r.nome || "") + "</b>! Aqui está o seu resumo:<br><br>Vendas de hoje: <b>" + (r.hoje || 0) + "</b> — R$" + (r.hoje_valor || "0,00") + " a receber amanhã<br>Total do mês: <b>" + (r.mes || 0) + "</b> vendas — R$" + (r.mes_valor || "0,00") + "<br>Já recebido: R$" + (r.recebido || "0,00") + "", function(){
